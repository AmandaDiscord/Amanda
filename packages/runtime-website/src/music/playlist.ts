...
commands.assign([
	{
		...
		async process(cmd, lang) {
			if (!common.queues.doChecks(cmd, lang)) return snow.interaction.editOriginalInteractionResponse(cmd.application_id, cmd.token, { content: lang.GLOBAL.MUSIC_DISABLED })
			...
			if (notNull.length === 0) return snow.interaction.editOriginalInteractionResponse(cmd.application_id, cmd.token, { content: langReplace(lang.GLOBAL.MUSIC_INVALID_ACTION, { username: cmd.author.username }) })
			...
			if (optionMeta !== null) {
				...
				if (notNull2.length === 0) return snow.interaction.editOriginalInteractionResponse(cmd.application_id, cmd.token, { content: langReplace(lang.GLOBAL.MUSIC_INVALID_ACTION, { username: cmd.author.username }) })
				...
				if (optionShow !== null) {
					...
					).then(r => r?.rows ?? []) as Array<{ playlist_id: string; name: string; author: string; play_count: number; count: number; length: number; ranking: string; }>
					...
					await sql.orm.insert("playlists", { name: optionCreate, author: cmd.author.id })
				...
				return snow.interaction.editOriginalInteractionResponse(cmd.application_id, cmd.token, { content: langReplace(lang.GLOBAL.PLAYLIST_SONG_REMOVED, { "title": toRemove.name }) })
				...
				if (optionFrom < optionTo) {
					await sql.orm.update("playlist_songs", { next: fromRow.next }, { playlist_id: fromRow.playlist_id, next: fromRow.video_id }) // update row before item
					await sql.orm.update("playlist_songs", { next: toRow.next }, { playlist_id: fromRow.playlist_id, video_id: fromRow.video_id }) // update moved item
					await sql.orm.update("playlist_songs", { next: fromRow.video_id }, { playlist_id: fromRow.playlist_id, video_id: toRow.video_id }) // update row before moved item
				} else if (optionFrom > optionTo) {
					await sql.orm.update("playlist_songs", { next: fromRow.next }, { playlist_id: fromRow.playlist_id, next: fromRow.video_id }) // update row before item
					await sql.orm.update("playlist_songs", { next: fromRow.video_id }, { playlist_id: fromRow.playlist_id, next: toRow.video_id }) // update row before moved item
					await sql.orm.update("playlist_songs", { next: toRow.video_id }, { playlist_id: fromRow.playlist_id, video_id: fromRow.video_id }) // update moved item
				...
				const userVoiceState = await sql.orm.get("voice_states", { user_id: cmd.author.id, guild_id: cmd.guild_id! })
				if (!userVoiceState) return snow.interaction.editOriginalInteractionResponse(cmd.application_id, cmd.token, { content: langReplace(lang.GLOBAL.VC_REQUIRED, { username: cmd.author.username }) })
				if (queue?.voiceChannelID && userVoiceState.channel_id !== queue.voiceChannelID) return snow.interaction.editOriginalInteractionResponse(cmd.application_id, cmd.token, { content: langReplace(lang.GLOBAL.MUSIC_SEE_OTHER, { channel: `<#${queue.voiceChannelID}>` }) })

				const node = (queue?.node ? common.nodes.byID(queue.node) ?? common.nodes.byIdeal() ?? common.nodes.random() : common.nodes.byIdeal() ?? common.nodes.random())
				if (!queue) {
				...
				const trackss = (optionShuffle ? sharedUtils.arrayShuffle(sliced) : sliced).map(row => new trackTypes.RequiresSearchTrack("!", { title: row.name, length: BigInt(row.length * 1000), identifier: row.video_id }, row.video_id, cmd.author, sharedUtils.getLang(cmd.guild_locale!)))
