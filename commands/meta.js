			...
			process: async function(msg, suffix) {
				...
				if (args[0].toLowerCase() == "self") {
					...
					if (args[1].toLowerCase() == "view") {
						...
						embed = new Discord.RichEmbed().setColor("36393E").setAuthor(`Settings for ${msg.author.tag}`, msg.author.smallAvatarURL).setDescription(`Waifu Alerts: ${memsettings.waifuAlert != 0} - Messages for waifu related things\nGambling Alerts: ${memsettings.gamblingAlert != 0} - Messages for gambling related things`);
						if (permissions && !permissions.has("EMBED_LINKS")) content = `${embed.author.name}\n${embed.description}`;
						else content = embed;
						return msg.channel.send(content);
					}
					...
					if (args[1].toLowerCase() == "view") {
						...
						embed = new Discord.RichEmbed().setColor("36393E").setAuthor(`Settings for ${msg.guild.name}`, msg.guild.iconURL).setDescription(`Waifu Alerts: ${guildsettings.waifuAlert != 0} - Messages for waifu related things\nGambling Alerts: ${guildsettings.gamblingAlert != 0} - Messages for gambling related things`);
						if (permissions && !permissions.has("EMBED_LINKS")) content = `${embed.author.name}\n${embed.description}`;
						else content = embed;
						return msg.channel.send(content);
					}
