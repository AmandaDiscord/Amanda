...
router.emit("help", music_info);
router.on("command", file_music);
router.once(__filename, () => {
	router.removeListener("command", file_music);
});
async function file_music(passthrough) {
	const { client, msg, cmd, suffix, utils } = passthrough;

	function musicCommandListener(action) {
		if (action == "getQueues") {
			router.emit("musicOut", "queues", queues);
	...
	if (cmd == "musictoken" || cmd == "token" || cmd == "webtoken") {
