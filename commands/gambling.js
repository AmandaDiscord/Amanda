...
router.emit("help", gambling_info);
router.on("command", file_gambling);
router.once(__filename, () => {
	router.removeListener("command", file_gambling);
});
async function file_gambling(passthrough) {
	...
	if (cmd == "dice") {
		return msg.channel.send(`You rolled a ${Math.floor(Math.random() * (6 - 1) + 1)} and a ${Math.floor(Math.random() * (6 - 1) + 1)}`);
	}


	else if (cmd == "slot" || cmd == "slots") {
		...
		let embed = new Discord.RichEmbed()
			...
			.setColor("F8E71C")
		msg.channel.send({embed});
		return member.send(`**${msg.author.tag}** has given you ${gift} <a:Discoin:422523472128901140>`).catch(() => msg.channel.send("I tried to DM that member but they may have DMs disabled from me"));
	}
		...
		member.send(`**${msg.author.tag}** has claimed you for ${claim} <a:Discoin:422523472128901140> ${face}`).catch(() => msg.channel.send(`I tried to DM a **${member.user.tag}** about the transaction but they may have DMs from me disabled`));
		let embed = new Discord.RichEmbed()
	...
	else if (cmd == "waifuleaderboard" || cmd == "waifulb") {
