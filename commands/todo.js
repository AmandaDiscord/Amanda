...
function extractTags(words, enableRemove = false) {
	...
	words.forEach(word => {
		if (word.startsWith("+") && word.length >= 2) {
			addTags.push(word.slice(1))
		} else if (enableRemove && word.startsWith("-") && word.length >= 2) {
			removeTags.push(word.slice(1))
...
function paginateResults(channel, itemStrings) {
	if (itemStrings.length === 0) {
		return channel.send("No matches.")
	}
		...
		process: async function(msg, suffix) {
			...
			if (subcommand === "add") {
				...
				if (name.trim().length === 0) {
					return msg.channel.send(`Usage: &todo ${subcommand} something really cool +tag +anothertag`)
				}
				let connection = await utils.getConnection()
				await utils.sql.all("INSERT INTO Todo (name, complete) VALUES (?, 0)", name, connection)
				let id = (await utils.sql.get("SELECT last_insert_id() AS id", [], connection)).id
				connection.release()
				tags.forEach(tag => {
				...
				if (!idString || isNaN(id)) {
					return msg.channel.send(`Usage: &todo ${subcommand} <id>`)
				}
				...
				if (!filter) {
					return msg.channel.send(`Usage: &todo ${subcommand} <tag> [anothertag...]`)
				}
				...
				let items
				if (filter) {
					items = await utils.sql.all("SELECT id, name, complete FROM Todo WHERE name LIKE ?", `%${filter}%`)
				} else {
					items = await utils.sql.all("SELECT id, name, complete FROM Todo")
				}
				const tags = await utils.sql.all("SELECT id, tag FROM TodoTags")
				const itemStrings = buildItemStrings(items, tags)
				paginateResults(msg.channel, itemStrings)
			} else {
				return msg.channel.send("Invalid subcommand. Options are: list, tag, tags, add, remove, complete, incomplete")
			}
		}
	}
})
