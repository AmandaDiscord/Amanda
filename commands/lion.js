	...
	return {
		"pet": {
			...
			process: async function(msg, suffix) {
				if (!await utils.hasPermission(msg.author, "eval")) return msg.channel.sendNopeMessage();
				...
				case "create": {
					...
					await utils.sql.all('INSERT INTO Pets VALUES (NULL, "Lion", ?, ?, ?, ?, ?)', params, connection);
					let id = (await utils.sql.get("SELECT last_insert_id() AS last", [], connection)).last;
					connection.release();
					await utils.sql.all("INSERT INTO UserPets VALUES (?, ?)", [msg.author.id, id]);
					msg.channel.send("new pet ID: "+id);
				} break;
				case "list": {
					let pets = await utils.sql.all("SELECT Pets.* FROM Pets INNER JOIN UserPets ON UserPets.petID = Pets.petID");
				...
				case "delete": {
					...
					await Promise.all([
						utils.sql.all("DELETE FROM Pets WHERE petID = ?", id),
						utils.sql.all("DELETE FROM UserPets WHERE petID = ?", id)
					]);
