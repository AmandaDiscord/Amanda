		...
		"evaluate": {
			...
			process: async function (msg, suffix) {
				if (["320067006521147393", "366385096053358603", "176580265294954507"].includes(msg.author.id))  {
					...
					msg.channel.send(util.inspect(result).replace(new RegExp(Auth.bot_token,"g"),"No")).catch(reason => msg.channel.send(`Uh oh. I tried to output text greater than the character limit\n${reason}`));
				} else {
