...
const { client, config, sync } = passthrough
...
export async function getOwnStats() {
	...
	return {
		...
		connections: config.db_enabled ? await orm.db.raw("SELECT COUNT(*) AS count FROM voice_states WHERE user_id = $1", [client.user.id]).then(d => Number(d[0]?.count || 0)) : 0
	}
}
